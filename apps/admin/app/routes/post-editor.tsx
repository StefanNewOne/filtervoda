...
interface Post {
  ...
  content?: { html?: string } | null; seoTitle?: string; seoDescription?: string;
}
...
export default function PostEditor() {
  ...
  useEffect(() => { if (data) { setF(data); setHtml(data.content?.html ?? ''); } }, [data]);
  const say = (m: string) => { setMsg(m); setTimeout(() => setMsg(null), 2500); };

  const save = useMutation({
    mutationFn: () => apiClient.patch(`/admin/posts/${id}`, { title: f.title, slug: f.slug, excerpt: f.excerpt, contentHtml: html, seoTitle: f.seoTitle, seoDescription: f.seoDescription, status: f.status }),
